 ...
 * Creates new aliases.
 */
TYPES.alias = (alias, original) => {
  // do not overwrite existing aliases
  if (!TYPE_ALIASES.hasOwnProperty(original)) {
    TYPE_ALIASES[original] = []
  }

  // avoid duplicate aliases
  if (TYPE_ALIASES[original].indexOf(alias) === -1) {
    TYPE_ALIASES[original].push(alias)
  }

  // return types handler for chaining
  return TYPES
}
